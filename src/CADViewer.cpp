...
void CADViewer::createPipline(Discrete_Shape dshape) {
    ...
    color = vsg::vec4Value::create(vsg::vec4{ 0.9, 0.9, 0.9, 1.0 });
    material = vsg::PhongMaterialValue::create();
    material->value().ambient.set(0.9, 0.9, 0.9, 1.0);
    material->value().diffuse.set(0.55, 0.55, 0.55, 1.0);
    material->value().specular.set(0.7, 0.7, 0.7, 1.0);
    //std::cout << mat->value().shininess;
    material->value().shininess = 35;
    ...
    graphicsPipelineConfig->assignArray(vertexArrays, "vsg_Vertex", VK_VERTEX_INPUT_RATE_VERTEX, dshape.vertices);
    graphicsPipelineConfig->assignArray(vertexArrays, "vsg_Normal", VK_VERTEX_INPUT_RATE_VERTEX, dshape.normals);
    //graphicsPipelineConfig->assignArray(vertexArrays, "vsg_TexCoord0", VK_VERTEX_INPUT_RATE_VERTEX, texcoords);

    graphicsPipelineConfig->assignArray(vertexArrays, "vsg_Color", VK_VERTEX_INPUT_RATE_INSTANCE, color);
    ...
    drawCommands->addChild(vsg::BindIndexBuffer::create(dshape.indexes));
    drawCommands->addChild(vsg::DrawIndexed::create(dshape.indexes->size(), 1, 0, 0, 0));
...
vsg::ref_ptr<vsg::Group> CADViewer:: createLight(vsg::ref_ptr<vsg::Group> vsgScene){
...
vsg::ref_ptr<vsg::ShaderSet> CADViewer::readShader(const vsg::Path& vertFile, const vsg::Path& fragFile) {
...
void CADViewer::createWindow(vsg::ref_ptr<vsg::Group> scene)
{
    //main window
...
void CADViewer::repaint(Discrete_Shape dshape) {
    //执行重新绘制的代码----------------------------------------------------------------
    auto graphicsPipelineConfig = vsg::GraphicsPipelineConfigurator::create(shader);
    vsg::DataList vertexArrays;
    //传入模型几何参数
